package com.emergentgameplay.oats;
// Wav file IO class
// A.Greensted
...
public class WavFile
{
	...
	public static WavFile newWavFile(OutputStream oStream, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
	...
	/**
	 * 
	 * @param iStream The inputstream for the wav
	 * @param length The length of the wav in bytes
	 * @return
	 * @throws IOException
	 * @throws WavFileException
	 */
	public static WavFile openWavFile(InputStream iStream, long length) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and store the file reference
		WavFile wavFile = new WavFile();

		// Create a new file input stream for reading file data
		wavFile.iStream = iStream;
		...
		if (length != chunkSize+8) {
			throw new WavFileException("Header chunk size (" + chunkSize + ") does not match stream size (" + length + ")");
		}
